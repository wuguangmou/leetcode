...
public class ExcelColumnNumber_171 {
    /**
     * 有了问题168的经验后，同样以进制转换的视角看待问题，这个问题就比较简单了<br/>
...
//        }
    ...
    /**
     * 在方案1中遍历的方式是从字母的低位向高位，即从字符的后面向前面遍历，如果反着遍历也是可以的。<br/>
     * 其实在十进制运算中我们经常使用的，比如要还原的数字是 2019，依次给你数字 2,0,1,9。就可以用下边的算法。<br/>
     * int res = 0;<br/>
     * res = res * 10 + 2; //2<br/>
     * res = res * 10 + 0; //20<br/>
     * res = res * 10 + 1; //201<br/>
     * res = res * 10 + 9; //2019<br/>
